...
export const FormattedNumber = ({ value, variant = "standard", currency = "EUR", withDigits = true }: FormattedNumberProps): JSX.Element => {
  if (value !== 0 && !value) {
    return <React.Fragment />
  }
  const additionalProps: any = withDigits ? {} : { minimumFractionDigits: 0, maximumFractionDigits: 0 }
  if (variant === "currency") {
    return (
    ...
    return <FormattedNumberReact value={value / 100} style={variant} minimumFractionDigits={withDigits ? 2 : 0} />
  } else {
